...
boost::filesystem::path StepUnzip::GenerateTmpDir() {
  boost::filesystem::path install_tmp_dir;
  const char* tmp_user = getuid() != tzplatform_getuid(TZ_SYS_GLOBALAPP_USER)
      ? tzplatform_getenv(TZ_USER_APP) : tzplatform_getenv(TZ_SYS_RW_APP);

  boost::filesystem::path tmp_dir(tmp_user);
