...
#include "ManifestParser/XmlNodes.h"
...
StepParse::process() {
  ...
  try {
    ...
    ManifestParser::Manifest m(mPath.c_str());
    setContextByManifest(m);
  } catch (FileNotFoundException *e) {
    delete e;   // Consume this exception here
    return status_error;
  } catch (ManifestParser::FileOpenFailureException *e) {
    delete e;
    return status_error;
  }
  return status_ok;
 ...
 */
void
StepParse::setContextByManifest(ManifestParser::Manifest &m) {
  ManifestParser::XmlNodeManifest &manifest = m.manifest;
  // debug
  std::cout << "[StepParse] Parse manifest xml values:";
  std::cout << "xmlns(" << manifest.xmlns << ") api_version(" << manifest.api_version << ") package(" << manifest.package << ") version(" << manifest.version << ")" << endl;
...
void
StepParse::setPkgInfoManifest(manifest_x* m, ManifestParser::XmlNodeManifest &manifest) {
  ...
  // Privileges
  vector<ManifestParser::XmlNodePrivilege *> vp = manifest.privileges.getPrivilegeVector();
  vector<ManifestParser::XmlNodePrivilege *>::iterator it;
  for(it = vp.begin(); it != vp.end(); it++) {
    privilege_x *p = (privilege_x *)calloc(1, sizeof(privilege_x));
