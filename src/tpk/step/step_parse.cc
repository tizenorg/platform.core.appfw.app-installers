...
bool StepParse::FillApplicationInfo(manifest_x* manifest) {
  ...
  // manifest->xmlns = strdup(app_info->xmlns().c_str());
...
// datacontrol
bool StepParse::FillDataControl(manifest_x* manifest) {
  std::shared_ptr<const DataControlInfoList> data_control_list =
    ...
    for (const auto& control : data_control_list->items) {
      datacontrol_x* data_control =
          static_cast<datacontrol_x*>(calloc(sizeof(datacontrol_x), 1));
      ...
      LISTADD(manifest->uiapplication->datacontrol, data_control);
    }
  return true;
}

// label
bool StepParse::FillLabel(manifest_x* manifest) {
  std::shared_ptr<const LabelInfoList> label_list =
    ...
    for (const auto& control : label_list->items) {
      label_x* label =
          static_cast<label_x*>(calloc(sizeof(label_x), 1));
...
bool StepParse::FillManifestX(manifest_x* manifest) {
  ...
  if (!FillAppControl(manifest))
    return false;
  if (!FillDataControl(manifest))
    return false;
  if (!FillLabel(manifest))
    return false;
