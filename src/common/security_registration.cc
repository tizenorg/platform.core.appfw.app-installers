...
bool RegisterSecurityContextForApps(
  ...
  for (uiapplication_x* ui = manifest->uiapplication;
    ...
    if (!RegisterSecurityContext(ui->appid, pkg_id, uid, path, manifest)) {
      return false;
    }
  }
  ...
  for (serviceapplication_x* svc =
    ...
    if (!RegisterSecurityContext(svc->appid, pkg_id, uid, path,  manifest)) {
      return false;
    }
  }
  return true;
...
bool UnregisterSecurityContextForApps(
    const std::string& pkg_id, uid_t uid, manifest_x* manifest) {
  for (uiapplication_x* ui = manifest->uiapplication;
      ui != nullptr; ui = ui->next) {
    if (!ui->appid) {
      return false;
    }
    if (!UnregisterSecurityContext(ui->appid, pkg_id, uid)) {
      return false;
    }
  }
  ...
  for (serviceapplication_x* svc =
    ...
    if (!UnregisterSecurityContext(svc->appid, pkg_id, uid)) {
      return false;
    }
  }
  return true;
