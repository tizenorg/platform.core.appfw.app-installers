...
bool PluginsLauncher::LaunchPlugins(
    const std::vector<std::string> &plugin_names, xmlDocPtr docPtr,
    ActionType action, const std::string &pkgId, int &result) {
  for (const auto &plugin : plugin_names) {
    LOG(DEBUG) << "Installing plugin " << plugin;
    std::unique_ptr<DynamicLibHandle> dl_handle(new DynamicLibHandle);
    if (!dl_handle->Create(GetPath(plugin), RTLD_LAZY | RTLD_LOCAL)) {
...
bool PluginsLauncher::GetName(ProcessType process, ActionType action,
  ...
  static std::map<std::pair<ActionType, ProcessType>, std::string> names {
    {{ActionType::Install,   ProcessType::PreInstall},  "PKGMGR_PARSER_PLUGIN_PRE_INSTALL"},
    {{ActionType::Upgrade,   ProcessType::PreInstall},  "PKGMGR_PARSER_PLUGIN_PRE_UPGRADE"},
    {{ActionType::Uninstall, ProcessType::PreInstall},  "PKGMGR_PARSER_PLUGIN_PRE_UNINSTALL"},
    {{ActionType::Install,   ProcessType::Install},     "PKGMGR_PARSER_PLUGIN_INSTALL"},
    {{ActionType::Upgrade,   ProcessType::Install},     "PKGMGR_PARSER_PLUGIN_UPGRADE"},
    {{ActionType::Uninstall, ProcessType::Install},     "PKGMGR_PARSER_PLUGIN_UNINSTALL"},
    {{ActionType::Install,   ProcessType::PostInstall}, "PKGMGR_PARSER_PLUGIN_POST_INSTALL"},
    {{ActionType::Upgrade,   ProcessType::PostInstall}, "PKGMGR_PARSER_PLUGIN_POST_UPGRADE"},
    {{ActionType::Uninstall, ProcessType::PostInstall}, "PKGMGR_PARSER_PLUGIN_POST_UNINSTALL"}
  };

  auto pos = names.find(std::pair<ActionType, ProcessType>{action, process});
  if (pos == names.end()) {
