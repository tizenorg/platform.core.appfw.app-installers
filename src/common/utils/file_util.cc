...
bool MoveFile(const bf::path& src, const bf::path& dst) {
  ...
  bf::rename(src, dst, error);
  if (error) {
    LOG(WARNING) << "Cannot move file: " << src << ". Will copy/remove... with error [" << error << "]";
    bf::copy_file(src, dst, bf::copy_option::overwrite_if_exists, error);
    if (error) {
      LOG(WARNING) << "Cannot copy file " << src << " due to error [" << error << "]";
      return false;
    }
    bf::remove_all(src, error);
    if (error) {
      LOG(ERROR) << "Cannot remove old file when coping: " << src << "with error [" << error << "]";
    }
  }
  return true;
}
