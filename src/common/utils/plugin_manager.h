...
class PluginManager {
 public:
  PluginManager(const std::string& xmlPath, const std::string& listPath)
      : xmlParser_(std::make_shared<PluginsXmlParser>(xmlPath)),
        listParser_(std::make_shared<PluginsListParser>(listPath)) {}
 ...
 private:
  ...
  std::shared_ptr<PluginsXmlParser> xmlParser_;
  std::shared_ptr<PluginsListParser> listParser_;
};
}  // namespace common_installer
