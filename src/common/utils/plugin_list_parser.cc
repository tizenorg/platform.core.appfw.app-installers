...
PluginInfo::PluginInfo(int flag, const std::string& type,
    ...
    : flag_(flag), type_(type), name_(name), path_(path) {}
...
std::string PluginsListParser::ExtractRaw(const std::string& data,
  ...
  if (match.size() != 2) {
    LOG(ERROR) << "Data no find during extract parameter";
    return {};
  }
  return match[1];
}
...
const std::vector<std::shared_ptr<PluginInfo>>& PluginsListParser::Data() {
  return data_;
}
...
void PluginsListParser::AddData(const std::shared_ptr<PluginInfo>& data) {
...
bool PluginsListParser::ParsePluginsRawData(
  ...
  for (std::string line : lines) {
    ...
    AddData(std::make_shared<PluginInfo>(_flag, type, name, path));
  }

  return true;
}

void PluginsListParser::Split(const std::string& line, const char* delim,
                              std::vector<std::string>* parts) {
  char* position = nullptr;
  char* token = strtok_r(const_cast<char*>(line.c_str()), delim, &position);
  while (token != nullptr) {
    parts->push_back(std::string(token));
    token = strtok_r(nullptr, delim, &position);
  }
}
