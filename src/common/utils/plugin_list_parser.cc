...
const std::string& PluginInfo::type() { return type_; }

const std::string& PluginInfo::name() { return name_; }

const boost::filesystem::path& PluginInfo::path() { return path_; }

// class PluginsListParser

inline int PluginsListParser::ConvertFlagToInt(const std::string& flag) {
  const int kConvertStringBase = 16;
  return std::strtoul(flag.c_str(), nullptr, kConvertStringBase);
}

inline boost::filesystem::path PluginsListParser::ConvertPathToBoostFormat(
