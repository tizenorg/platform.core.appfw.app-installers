...
AppInstaller::AppInstaller(pkgmgr_installer *pi, const char* package_type)
  ...
  context_->request_type(request_type);
  context_->pkg_type(package_type);
  switch (request_type) {
    case PKGMGR_REQ_INSTALL:
     context_->file_path(pkgmgr_installer_get_request_info(pi));
     context_->pkgid(STR_EMPTY);
    break;
    case PKGMGR_REQ_UNINSTALL:
     context_->pkgid(pkgmgr_installer_get_request_info(pi));
     context_->file_path(STR_EMPTY);
    break;
  }
}
