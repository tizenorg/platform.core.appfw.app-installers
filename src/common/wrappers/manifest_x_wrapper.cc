...
ManifestXWrapper::ManifestXWrapper(const boost::filesystem::path& path,
  ...
  utilities_ =
      std::shared_ptr<ManifestXUtilities>(new ManifestXUtilities(object_));
...
void ManifestXWrapper::SetType(TypeApp type) {
  type_ = std::shared_ptr<WrapperStringType>(new WrapperStringType());
