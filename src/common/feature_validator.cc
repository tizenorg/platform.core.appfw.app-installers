...
bool FeatureValidator::ValidateBoolean(
  ...
  if ((value.empty() || value == kTrueFeature) && !supported) {
    *error = std::string("Feature '") + feature + " is not supported";
    return false;
  }
  return true;
}
...
bool FeatureValidator::ValidateInteger(
  ...
  int required_minimum = 0;
  try {
    required_minimum = std::stoi(value);
  } catch (const std::logic_error&) {
     return false;
  }
  if (required_minimum > platform_value) {
    *error = std::string("Feature '") + feature + "' has value '" +
        value + "' while platform declares only '" +
...
bool FeatureValidator::ValidateString(
  ...
  if (value != text) {
    *error = std::string("Feature '") + feature + "' has value '" + value +
        "' while platform declares '" + std::string(text) + "'";
    return false;
  }
  return true;
}
