...
bool PackageExternalStorage::Finalize(bool success) {
  ...
  case RequestType::Uninstall: {
    ret = handle_->interface.post_uninstall(pkgid_.c_str(), uid_);
    break;
  }
  default:
...
bool PackageExternalStorage::Initialize(
  ...
  switch (type_) {
  case RequestType::Install:
    ret = handle_->interface.pre_install(pkgid_.c_str(), glist,
                                         external_size, uid_);
    break;
  case RequestType::Clear:  // lets say we are "upgrading fs" in this case
  case RequestType::Update:
  case RequestType::Delta:
    ret = handle_->interface.pre_upgrade(pkgid_.c_str(), glist,
                                         external_size, uid_);
    break;
  case RequestType::Uninstall:
    ret = handle_->interface.pre_uninstall(pkgid_.c_str(), uid_);
    break;
...
bool PackageExternalStorage::IsRequestedPackageInstalled() const {
  int ret = app2ext_get_app_location(pkgid_.c_str(), uid_);
  switch (ret) {
