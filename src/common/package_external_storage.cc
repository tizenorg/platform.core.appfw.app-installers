...
bool PackageExternalStorage::Finalize(bool success) {
  ...
  case RequestType::Install: {
    ret = handle_->interface.client_post_install(pkgid_.c_str(),
        success ? APP2EXT_STATUS_SUCCESS : APP2EXT_STATUS_FAILED);
    break;
  }
  case RequestType::Update: {
    ret = handle_->interface.client_post_upgrade(pkgid_.c_str(),
        success ? APP2EXT_STATUS_SUCCESS : APP2EXT_STATUS_FAILED);
    break;
  }
  case RequestType::Uninstall: {
    ret = handle_->interface.client_post_uninstall(pkgid_.c_str());
    break;
  }
  default:
...
bool PackageExternalStorage::Initialize(
  ...
  switch (type_) {
  case RequestType::Install:
    ret = handle_->interface.client_pre_install(pkgid_.c_str(), glist,
                                                external_size);
    break;
  case RequestType::Clear:  // lets say we are "upgrading fs" in this case
  case RequestType::Update:
  case RequestType::Delta:
    ret = handle_->interface.client_pre_upgrade(pkgid_.c_str(), glist,
                                                external_size);
    break;
  case RequestType::Uninstall:
    ret = handle_->interface.client_pre_uninstall(pkgid_.c_str());
    break;
