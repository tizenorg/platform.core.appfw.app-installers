/* 2014, Copyright © Eurogiciel Coporation, license APACHE-2.0, see LICENSE file */
...
int StepParse::process(ContextInstaller* data) {
  ...
  xmlInitParser();
  manifest_x* mfx = pkgmgr_parser_usr_process_manifest_xml(data->xml_path().c_str(), data->uid());
  if (!mfx) {
