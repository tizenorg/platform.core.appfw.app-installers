...
Step::Status StepMountInstall::process() {
  ...
  bf::path zip_destination_path =
      GetZipPackageLocation(context_->pkgid.get(),
  ...
  if (!tzip_final.MountZip(zip_destination_path.string())) {
...
Step::Status StepMountInstall::undo() {
  bs::error_code error;
  bf::remove(GetZipPackageLocation(context_->pkgid.get(),
                                   context_->is_preload_request.get()), error);
