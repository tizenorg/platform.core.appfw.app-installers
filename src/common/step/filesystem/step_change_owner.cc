...
namespace {
  bool SetOwner(const boost::filesystem::path& subpath, uid_t uid,
                                              gid_t gid) {
  ...
  bool SetOwnerForSymbolic(const boost::filesystem::path& subpath, uid_t uid,
                                              gid_t gid) {
...
Step::Status StepChangeOwner::process() {
  ...
  int ret = getpwuid_r(context_->uid.get(), &pwd, buf_pw, sizeof(buf_pw), &pwd_result);
  if (ret != 0 || pwd_result == nullptr) {
    LOG(WARNING) << "Failed to get user for home directory: " << context_->uid.get();
    return Status::ERROR;
  }
  ...
  bf::path start_path = context_->root_application_path.get() / context_->pkgid.get();
  ...
  // For icon files
  start_path = getIconPath(context_->uid.get(), context_->is_preload_request.get());
