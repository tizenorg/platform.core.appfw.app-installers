...
int StepSignal::process(ContextInstaller* data) {
  sendSignal(data, PKGMGR_INSTALLER_START_KEY_STR, PKGMGR_INSTALLER_INSTALL_EVENT_STR);
  DBG("Send Start");
  return APPINST_R_OK;
}
...
int StepSignal::clean(ContextInstaller* data) {
   ...
   sendSignal(data, PKGMGR_INSTALLER_END_KEY_STR,PKGMGR_INSTALLER_OK_EVENT_STR);
  DBG("Send Sucess");
  return APPINST_R_OK;
}

int StepSignal::undo(ContextInstaller* data) {
   sendSignal(data, PKGMGR_INSTALLER_END_KEY_STR,PKGMGR_INSTALLER_FAIL_EVENT_STR);
  DBG("Send Error");
  return APPINST_R_OK;
}
