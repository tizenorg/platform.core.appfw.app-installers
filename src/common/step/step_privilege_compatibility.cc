...
bool TranslatePrivilegesForCompatibility(manifest_x* m) {
  ...
  if (privilege_manager_get_mapped_privilege_list(m->api_version, is_webapp ? PRVMGR_PACKAGE_TYPE_WRT : PRVMGR_PACKAGE_TYPE_CORE, m->privileges, &mapped_privilege_list)
      != PRVMGR_ERR_NONE) {
