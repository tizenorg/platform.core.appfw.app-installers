...
using CompositeSpec =
    common_installer::CompositeSpecification<ci_sec::BackgroundCatSpecTest>;
...
class VersionAtLeastSpec : public CompositeSpec {
 public:
  explicit VersionAtLeastSpec(utils::VersionNumber const& version)
      : version_(version) {}

  bool IsSatisfiedBy(const ci_sec::BackgroundCatSpecTest &candidate) override {
...
class KnownValueSpec : public CompositeSpec {
 public:
  explicit KnownValueSpec(const ci_sec::BackgroundCatSet &values) :
      valid_values_(values) {
  }

  bool IsSatisfiedBy(const ci_sec::BackgroundCatSpecTest &candidate) override {
    ci_sec::BackgroundCatSet intersect;
 ...
 private:
  ci_sec::BackgroundCatSet valid_values_;
};
...
class UnknownValueSpec : public CompositeSpec {
 public:
  explicit UnknownValueSpec(const ci_sec::BackgroundCatSet &values) :
    invalid_values_(values) {
  }

  bool IsSatisfiedBy(const ci_sec::BackgroundCatSpecTest &candidate) override {
    ci_sec::BackgroundCatSet diff;
 ...
 private:
  ci_sec::BackgroundCatSet invalid_values_;
};
...
BaseStepCheckBackgroundCategory::BaseStepCheckBackgroundCategory(
  ...
  auto ver24 = std::make_shared<VersionAtLeastSpec>(
      utils::VersionNumber("2.4"));
  auto certUntrusted =
  ...
  IBackgroundCatSpecPtr certTrusted = certPartner->Or(certPlatform);
  sentAllSpec_ = ver24->Not()->And(
      background_supt->And(certPublic->And(system->Not())->Or(certTrusted)));
  failSpec_ = certPublic->And(system)->Or(certUntrusted->And(empty->Not()));
...
common_installer::Step::Status BaseStepCheckBackgroundCategory::process() {
  ...
  for (application_x* app :
    ...
    BackgroundCatSpecTest test = { version,
                                   privilege_level,
                                   background_supt,
                                   std::move(background_cat) };
