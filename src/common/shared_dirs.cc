...
const char kExternalStoragePrivilege[] = "http://tizen.org/privilege/externalstorage.appdata";
...
bool PerformExternalDirectoryCreationForAllPkgs(void) {
  ...
  for (auto l  : list) {
    ...
    ret = pkgmgrinfo_pkginfo_filter_foreach_pkginfo(filter_handle, privilege_cb, (void *)uid);
    if (ret != PMINFO_R_OK) {
