...
bool CreateUserDirectories(const std::string& user, const std::string& pkgid,
...
bool CreatePerUserDirectories(const std::string& pkgid,
  ...
  for (bf::directory_iterator iter("/home"); iter != bf::directory_iterator();
    ...
    if (!CreateUserDirectories(user, pkgid, author_id, api_version, path))
      return false;
  }
  return true;
}
...
bool PerformInternalDirectoryCreationForUser(const char* user_name,
                                             ...
                                             const std::string& api_version) {
  std::string path = "";
  return CreateUserDirectories(user_name, pkgid, author_id, api_version, path);
}
...
bool PerformInternalDirectoryCreationForAllUsers(const std::string& pkgid,
  ...
  for (bf::directory_iterator iter("/home"); iter != bf::directory_iterator();
    ...
    const bf::path& home_path = iter->path();
    std::string user = home_path.filename().string();
    if (!PerformInternalDirectoryCreationForUser(user.c_str(), pkgid,
                                                 author_id, api_ver))
...
bool PerformExternalDirectoryCreationForAllUsers(const std::string& pkgid,
  ...
  for (bf::directory_iterator iter("/home"); iter != bf::directory_iterator();
    ...
    const bf::path& home_path = iter->path();
    std::string user = home_path.filename().string();
    if (!PerformExternalDirectoryCreationForUser(user.c_str(), pkgid,
                                                 author_id, api_ver))
