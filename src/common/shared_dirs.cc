...
bool CreateSymlinksForGlobalApp(const std::string& pkgid) {
  ...
  user_list list = GetUserList();
  for (auto l : list) {
    LOG(DEBUG) << "Creating symlinks for uid: " << std::get<0>(l);
    bf::path apps_rw(std::get<2>(l) / "apps_rw");
    ...
    if (!CreateSymlinkFiles(src_dir, dst_dir, std::get<0>(l),
        std::get<1>(l)))
      continue;
  }
  return true;
...
bool DeleteSymlinksForGlobalApp(const std::string& pkgid) {
  ...
  user_list list = GetUserList();
  for (auto l : list) {
    LOG(DEBUG) << "Deleting symlinks for uid: " << std::get<0>(l);
    bf::path apps_rw(std::get<2>(l) / "apps_rw");
    bf::path dst_dir = apps_rw / pkgid;
    if (!bf::exists(dst_dir)) {
      LOG(ERROR) << "dst_dir not exists";
