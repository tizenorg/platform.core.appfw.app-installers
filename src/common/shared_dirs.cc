...
const char kInternalStorageDirPrefix[] = "/home";
...
ci::PkgList GetPkgInformation(const std::string& pkgid) {
  if (!ValidateTizenPackageId(pkgid)) return ci::PkgList();
...
bool PerformExternalDirectoryCreationForUser(uid_t user,
  ...
  const char* storage_path = tzplatform_mkpath(TZ_SYS_MEDIA,
                                               kExternalStorageDirPrefix);
  if (!CreateUserDirectories(user, pkgid, author_id, api_version, storage_path))
    return false;
  if (!CreateSkelDirectories(pkgid))
    return false;
  return true;
...
bool PerformInternalDirectoryCreationForAllUsers(const std::string& pkgid,
  ...
  for (bf::directory_iterator iter("/home"); iter != bf::directory_iterator();
    ...
    if (!PerformInternalDirectoryCreationForUser(pwd->pw_uid, pkgid,
                                                 author_id, api_ver))
...
bool PerformExternalDirectoryCreationForAllUsers(const std::string& pkgid,
  ...
  for (bf::directory_iterator iter("/home"); iter != bf::directory_iterator();
    ...
    if (!PerformExternalDirectoryCreationForUser(pwd->pw_uid, pkgid,
                                                 author_id, api_ver))
...
ci::PkgList CreatePkgInformationList(const std::vector<std::string>& pkgs) {
  return pkgs.empty() ?
