...
ExternalStorage::ExternalStorage(const std::string& pkgid) {
  LOG(DEBUG) << "ExternalStorage initialization started";
  pkgid_ = pkgid;
...
ExternalStorage::~ExternalStorage() {
  LOG(DEBUG) << "ExternalStorage deinitialization started";
  if (!IsInitialized()) {
    return;
  }
...
bool ExternalStorage::IsInitialized() {
  return initialization_;
}

bool ExternalStorage::PostInstallation(bool execute) {
  LOG(DEBUG) << "ExternalStorage postinstallation started";
  ...
  case APP2EXT_STATUS_FAILED:
    ...
    return false;
  default:
    LOG(ERROR) << "App2Ext postinstallation error " << ret;
    return false;
  }
}

bool ExternalStorage::PostUninstallation() {
  LOG(DEBUG) << "ExternalStorage postuninstallation started";
...
bool ExternalStorage::PreUninstallation() {
  LOG(DEBUG) << "ExternalStorage preuninstallation started";
  ...
  case APP2EXT_STATUS_FAILED:
    ...
    return false;
  default:
    LOG(ERROR) << "App2Ext preuninstallation error";
    return false;
  }
}

bool ExternalStorage::PreInstallation() {
  LOG(DEBUG) << "ExternalStorage preinstallation started";
  common_installer::ContextInstaller context;
  int size = common_installer::GetUnpackedPackageSize(context.file_path.get());
