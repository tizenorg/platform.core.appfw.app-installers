...
WidgetManifestParser::WidgetManifestParser(const bf::path& path) :
  manifest_path_(path) {
  if (!ParseManifest() && error_.empty())
    SetError("Parsing manifest failed with unknown error.", &error_);
}

bool WidgetManifestParser::IsValid() {
  return error_.empty();
}
...
bool WidgetManifestParser::StoreManifest(manifest_x* manifest) {
...
bool WidgetManifestParser::ParseManifest() {
  if (manifest_path_.empty()) {
    SetError(kErrMsgNoPath, &error_);
    return false;
  }
  if (!exists(manifest_path_)) {
    SetError(kErrMsgInvalidPath, &error_);
    return false;
  }

  manifest_ = parser::LoadManifest(
      manifest_path_.string(), parser::Manifest::TYPE_WIDGET, &error_);
  if (!manifest_)
    return false;
