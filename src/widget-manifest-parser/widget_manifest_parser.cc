...
bool WidgetManifestParser::FillManifestX(manifest_x* manifest) {
  ...
  manifest->uiapplication->appid = strdup(id_.c_str());
  manifest->uiapplication->type = strdup("webapp");

  // package data
  manifest->package = strdup(package_.c_str());
  manifest->mainapp_id = strdup(id_.c_str());
  manifest->version = strdup(version_.c_str());
  ...
  manifest->label->name = strdup(name_.c_str());
