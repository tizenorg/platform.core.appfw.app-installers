...
class VersionAtLeastSpec :
 ...
 public:
  explicit VersionAtLeastSpec(unsigned int version) : version_(version) {}

  bool IsSatisfiedBy(const BackgroundCatSpecTest &candidate) override {
    return candidate.version >= version_;
  }
...
common_installer::Step::Status StepCheckBackgroundCategory::process() {
  ...
  for (application_x* app :
    ...
    LOG(DEBUG) << "Validating background category";
    LOG(DEBUG) << "API Version: " << str_ver << "(" << req_ver << ")";
    LOG(DEBUG) << "Certificate: " << common_installer::PrivilegeLevelToString(
        privilege_level);
    LOG(DEBUG) << "Background-support: " << (bkgnd_supt ? "true" : "false");
    LOG(DEBUG) << "Found entries: " << bkgnd_cat.size();

    g_list_free_full(app->background_category, g_free);
    app->background_category = nullptr;

    BackgroundCatSpecTest test =
        { req_ver, privilege_level, bkgnd_supt, bkgnd_cat };
    ...
    if (sentAllSpec->IsSatisfiedBy(test)) {
      LOG(DEBUG) << "Insert all known background-categories";
    ...
    if (systemSpec->IsSatisfiedBy(test)) {
      LOG(DEBUG) << "Insert system background-category";
      app->background_category = g_list_append(
          app->background_category, strdup("system"));
    }

    if (knownSpec->IsSatisfiedBy(test)) {
      LOG(DEBUG) << "Insert parsed known background-categories";
    ...
    if (unknownSpec->IsSatisfiedBy(test)) {
      LOG(DEBUG) << "Insert parsed unknown background-categories";
      std::set<std::string> to_insert;
    ...
    LOG(DEBUG) << "No of returned entries: " << g_list_length(
        app->background_category);
  }
