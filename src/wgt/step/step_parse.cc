...
bool StepParse::FillManifestX(manifest_x* manifest) {
  std::string api_version, short_name;
  ...
  std::set<std::string> privileges = ExtractPrivileges(perm_info);
...
common_installer::Step::Status StepParse::process() {
  ...
  // we have to make ManifestHandler for icons
//  !manifest.Get(kWidgetIconFullKey, &key_value)
