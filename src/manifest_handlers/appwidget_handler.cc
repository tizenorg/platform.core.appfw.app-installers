...
bool GetMandatoryDictionary(const parser::Manifest& manifest,
    const std::string& key, const parser::utils::DictionaryValue** dict,
    std::string* error) {
  assert(dict);
...
// parameter is specified, it is also filled with proper message.
template <typename ValueType>
bool GetMandatoryValue(const parser::utils::DictionaryValue& dict,
...
template <typename ValueType>
bool GetOptionalValue(const parser::utils::DictionaryValue& dict,
...
bool ParseEachInternal(const parser::utils::Value& value,
...
bool ParseEach(const parser::utils::DictionaryValue& dict,
  ...
  if (value->IsType(parser::utils::Value::TYPE_DICTIONARY)) {
    if (!ParseEachInternal(*value, key, parse_single, data_container, error))
      return false;
  } else if (value->IsType(parser::utils::Value::TYPE_LIST)) {
    const parser::utils::ListValue* list;
    if (!value->GetAsList(&list)) {
      SetError(kErrMsgInvalidList, key, error);
      return false;
    }
    for (const parser::utils::Value* value : *list)
      if (!ParseEachInternal(*value, key, parse_single, data_container, error))
        return false;
  }
...
bool VerifyElementNamespace(const parser::utils::DictionaryValue& dict,
...
bool ParseLabel(const parser::utils::DictionaryValue& dict,
...
bool ParseIcon(const parser::utils::DictionaryValue& dict,
    const std::string& key, AppWidget* app_widget, std::string* error) {
  assert(app_widget);
...
bool ParseContentSizes(const parser::utils::DictionaryValue& dict,
    const std::string& key, AppWidget* app_widget, std::string* error) {
  assert(app_widget);
...
// Parses pd part
bool ParseContentDropView(const parser::utils::DictionaryValue& dict,
    const std::string& key, AppWidget* app_widget, std::string* error) {
  assert(app_widget);
...
bool ParseContent(const parser::utils::DictionaryValue& dict,
    const std::string& key, AppWidget* app_widget, std::string* error) {
  assert(app_widget);
...
bool ParseAppWidget(const parser::utils::DictionaryValue& dict,
...
bool AppWidgetHandler::Parse(
    ...
    std::string* error) {
  const parser::utils::DictionaryValue* dict = nullptr;
