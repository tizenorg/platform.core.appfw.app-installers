...
ci::pkg_list GetPackageListFromArgs(const bpo::variables_map& opt_map) {
  ...
  return ci::CreatePkgInformationList(pkgs);
}
  ...
  for (auto& p : pkgs) {
    switch (dir_operation) {
      case DirectoryOperation::CREATE_INTERNAL: {
        ...
        ci::PerformInternalDirectoryCreationForAllUsers(p.pkg_id, p.api_version,
                                             p.author_id);
      ...
      case DirectoryOperation::CREATE_EXTERNAL: {
        LOG(DEBUG) << "Running external directory creation for package id: "
            << p.pkg_id;
        ci::PerformExternalDirectoryCreationForAllUsers(p.pkg_id, p.api_version,
                                             p.author_id);
      }
      break;
