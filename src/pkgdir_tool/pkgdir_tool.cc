...
DirectoryOperation ParseDirectoryOptions(bpo::variables_map opt_map) {
...
ci::pkg_list GetPackageListFromArgs(const bpo::variables_map& opt_map) {
  bool allglobalpkgs = opt_map.count(kAllPkgIds) != 0;

  if (allglobalpkgs) {
    return ci::GetAllGlobalApps();
  }

  if (opt_map.count(kSinglePkgId)) {
    std::string pkgid = opt_map[kSinglePkgId].as<std::string>();
    if (ci::ValidateTizenPackageId(pkgid)) {
      return ci::GetPkg(pkgid);
    }
  }
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    ExclusiveOptions(opt_map,
                     kCreateInternalMode, kCreateExternalMode, kDeleteMode);
    ExclusiveOptions(opt_map,
                     kSinglePkgId, kAllPkgIds);
    bpo::notify(opt_map);
